...
DATA_DIR = os.path.join(SAVE_DIR, "scraped_article_headless")
...
class LaunchBrowserNode(Node):
    ...
    def execute(self, context: WorkflowContext) -> bool:
        ...
        # Run in headless=True mode to test!
        print(f"🚀 Launching CloakBrowser in headless=True mode and navigating to {URL}...")
        context.browser = launch(headless=True, humanize=True)
...
class NavigateToArticleNode(Node):
    ...
    def execute(self, context: WorkflowContext) -> bool:
        ...
        if not target:
            raise ValueError("No target article chosen.")
...
class DownloadImagesNode(Node):
    ...
    def execute(self, context: WorkflowContext) -> bool:
        ...
        target = context.target_article
        if not data:
            raise ValueError("No scraped data available.")
...
class CaptureSnapshotNode(Node):
    ...
    def execute(self, context: WorkflowContext) -> bool:
        page = context.page
        snapshot_path = os.path.join(DATA_DIR, "article_snapshot_headless.png")
        ...
        top_screenshot_path = os.path.join(SAVE_DIR, "article_snapshot_headless.png")
        with open(top_screenshot_path, "wb") as f:
            f.write(buf)
            
        context.snapshot_path = snapshot_path
        print(f"  ✅ Saved headless snapshot to: {top_screenshot_path}")
        return True
...
class WorkflowRunner:
    ...
    def run(self) -> bool:
        print("\n🏁 Starting Headless=True Scraper Workflow...")
        ...
        print("\n" + "="*60)
        print("🎉 Headless Workflow completed successfully!")
        print("="*60)
        return True
