...
class LaunchBrowserNode(Node):
    ...
    def execute(self, context: WorkflowContext) -> bool:
        ...
        print(f"🚀 Launching CloakBrowser (headless={self.headless}) and navigating to {context.url}...")
        context.browser = launch(headless=self.headless, humanize=True)
        context.page = context.browser.new_page()
        
        # Initial navigation
        context.page.goto(context.url, timeout=60000, wait_until="domcontentloaded")
        return True
...
class ScrapeArticleContentNode(Node):
    ...
    def execute(self, context: WorkflowContext) -> bool:
        ...
        article_json_path = os.path.join(context.output_dir, "article.json")
    ...
    def execute(self, context: WorkflowContext) -> bool:
        ...
        for idx, img_url in enumerate(data['images'][:3]):
            try:
                ...
                img_path = os.path.join(context.output_dir, img_name)
...
class CaptureSnapshotNode(Node):
    ...
    def execute(self, context: WorkflowContext) -> bool:
        page = context.page
        snapshot_path = os.path.join(context.output_dir, "article_snapshot.png")
        ...
        print(f"  ✅ Saved snapshot ({len(buf)} bytes) to: {snapshot_path}")
        return True
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Modular Node-based Scraper with Cloudflare Bypass")
    parser.add_argument("--headless", action="store_true", help="Run browser in headless mode (default: False)")
    parser.add_argument("--url", type=str, default="https://www.spot.ph", help="Target homepage URL (default: https://www.spot.ph)")
    parser.add_argument("--output", type=str, default="scraped_data", help="Directory to save scraped article, images, and screenshot (default: scraped_data)")
    
    args = parser.parse_args()
    
    # Establish context
    context = WorkflowContext(url=args.url, output_dir=args.output)
